...
class EmpController extends Controller
{
    ...
    public function processEmployee(Request $request)
    {
        ...
        $user->email    = str_replace(' ', '_', $request->emp_name) . Config::get('constants.options.default_user_email');
...
\Log::info($row->role);
                    ...
                    $user->email    = str_replace(' ', '_', $row->emp_name) . Config::get('constants.options.default_user_email');
                    $user->password = bcrypt('123456');
                    $user->save();
